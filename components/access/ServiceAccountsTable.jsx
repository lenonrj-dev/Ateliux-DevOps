// file: components/access/ServiceAccountsTable.jsx
"use client";
...
export default function ServiceAccountsTable({
  ...
  sortBy,
  onSortBy,
  // opcional: exibe skeletons durante carregamento
  loading = false,
}) {
  ...
  const gridCols = "grid grid-cols-[36px_minmax(220px,1.2fr)_1fr_1fr_120px]";
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-white/10">
      ...
      <div className={`${gridCols} px-3 py-2 text-xs uppercase tracking-wide text-gray-400 bg-white/5 hidden sm:grid`} role="row">
        ...
        <div role="columnheader">{headerBtn("Tokens", "tokens")}</div>
        ...
        <AnimatePresence initial={false}>
          ...
          {!loading && empty && (
            <motion.li
              ...
              role="status"
            >
              Nenhuma ServiceAccount encontrada para os filtros atuais.
            </motion.li>
          )}
              ...
              >
                ...
                <div className={`${gridCols} items-center hidden sm:grid`}>
                  <div>
                    <input
                      ...
                      aria-label={`Selecionar ${sa.name}`}
                    />
                  </div>
                  <button
                    onClick={() => onOpen(sa.id)}
                    onKeyDown={(e) => onRowKey(e, sa.id)}
                    className="flex flex-col text-left focus:outline-none focus:ring-2 focus:ring-cyan-500/40 rounded"
                    aria-label={`Abrir detalhes de ${sa.name}`}
                  >
                    <span className="text-sm font-medium text-gray-100">{sa.name}</span>
                    <span className="text-[11px] text-gray-400">Segredos: {sa.secrets?.length ?? 0}</span>
                  </button>
                  <div className="text-sm text-gray-300">{sa.namespace}</div>
                  <div className="text-sm text-gray-300">{sa.tokens?.length ?? 0}</div>
                  <div className="text-sm text-gray-300">{sa.age}</div>
                </div>
                ...
                >
                  <div className="flex items-center justify-between">
                    <div className="text-sm font-medium text-gray-100">{sa.name}</div>
                    <input
                      type="checkbox"
                      className="accent-cyan-500"
                      checked={selectedIds.includes(sa.id)}
                      onChange={(e) => {
                        e.stopPropagation();
                        onToggleSelect(sa.id);
                      }}
                      aria-label={`Selecionar ${sa.name}`}
                    />
                  </div>
                  <div className="text-[11px] text-gray-400">Segredos: {sa.secrets?.length ?? 0}</div>
                  <div className="text-sm text-gray-300">ns: {sa.namespace}</div>
                  <div className="text-sm text-gray-300">tokens: {sa.tokens?.length ?? 0}</div>
                  <div className="text-sm text-gray-300">idade: {sa.age}</div>
                </div>
              </motion.li>
            ))}
        </AnimatePresence>
